...
let prepare_proving () =
  let Ctx.{ module_name; handler_type_prefix; parameters; provenance = _; recursor; compiled_motive; _ }
      =
    Ctx.get ()
  in
  ...
  let handler_wrapper = 
    Codegen.wrap_module 
      ~module_name:handler_type_prefix
      ~inner_module:handler_types
      ~ctx:parameters
  in 
...
let open_theorem ~(name : Names.Id.t) ~(inductive : Libnames.qualid)
  ...
  let handler_names = 
      Termutils.handler_types_table name recursor 
      |> List.map fst 
  in
...
let open_theorem_extension ~name =
  ...
  let goal =
    Termutils.calculate_inductive_proof_goal ~theorem_name:name
      ~handler_type_prefix
      ~handler_names:implementing_handler_names
      ~suffix
  in
  let module_name = Naming.fresh_name ~prefix:(Names.Id.to_string name) in
  let goal_name = Naming.fresh_name ~prefix:"Goal" in
...
let close_theorem () =
  ...
  let f x = 
    x |> Termutils.cbn_type_check |> Termutils.reflect_checked_term    
  in  
  let all_handlers = List.map (fun (name, expr) -> name, f expr) all_handlers in 
  ...
  let compiled_impl =
    Codegen.compile_theorem_implementation ~name ~parameters
