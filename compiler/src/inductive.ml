...
let add_inductive_definition inductive =
  ...
  let inductive =
    ...
    | Some (further, InductiveDefinition { inductive = further_inductive; _ }), None ->
        (* Further binding *)
        let base =
    ...
    | Some (_, InductiveDefinition _), Some (_, InductiveDefinition _) ->
       ...
       Errors.fail
         ~info:"Further binding and inheritance at the same time has not been implememnted for inductive types"
    | _, _ -> Errors.fail ~info:"An inductive type can only be extended by another inductive type"
  in
  ...
  let compiled_signature =
    Codegen.compile_inductive_signature ~ind_def:inductive ~ctx:parameters ~family_name
  in
  let compiled_impl =
