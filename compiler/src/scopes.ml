...
let top_uninherited_fields judgement =
  ...
  let inherit_all_remained () =
    ...
    in
    let derived =
      FamilyType.{ name = judgement.derived.name;
                   body = judgement.derived.body @ types }
    in
  ...
  let apply_judgement_to_family_term ~(judgement : InhJudgement.t)
    ...
    let compute_family_term_elem (name, type_elem, inh_elem) =
      ...
      let _term_elem =
        family_term.body
        |> List.map fst
        |> List.find_opt (Names.Id.equal name)
      in 
    ...
    let body =
      judgement
      |> InhJudgement.family_type_inh_op
      |> List.map compute_family_term_elem        
    in
    FamilyTerm.{ body }
  ...
  let close_current_inheritance_judgement () =
    ...
    let InhJudgement.
          { base = base_family_type; derived = derived_family_type; _ } =
      judgement
    in
    let base_family =
