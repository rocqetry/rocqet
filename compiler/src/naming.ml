...
let _point_optionqualid (f : Names.Id.t) (path : Libnames.qualid option) : Libnames.qualid = 
  match path with 
  | None -> Libnames.qualid_of_ident f 
  | Some x -> _point_qualid_ f x

let _qualid_point_ (path : Libnames.qualid option) (f : Names.Id.t) : Libnames.qualid = 
  match path with 
  | Some path ->
    let path, base =  Libnames.repr_qualid path in 
...
let rename_ind_constructors
      (constructors : Vernacexpr.constructor_expr list)
