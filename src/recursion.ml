...
let close_recursion () =
  ...
  in
  let _ =
    Checks.check_exhaustive ~names ~inductive ~inductive_paths
  ...
  let module_name =
    let name =
      Nameops.add_suffix
        (Nameops.add_prefix family (Naming.concat_names names))
        "Ctx"
    in
  ...
  let behaviour_table =
    ...
    |> List.map (fun handler ->
           ( handler,
             Naming.computational_axiom_name ~recursor_names:names
...
let open_recursion ~(args : Frec_arg.t list) ~(suffix : RecKind.t)
    ~(arguments : Names.Id.t list) =
  let rec split3 xs =
    match xs with
    | [] -> ([], [], [])
...
let extend_argumets_with_inductive_case ~(inductive_names : Names.Id.t list)
    ...
    ~(arguments : Names.Id.t list) ~(inductive : VernacInductive.t) =
  let types =
    Termutils.flatten_inductive_constructor_type ~inductive_names ~inductive
      ~constructor
  in
  ...
  let result =
    ...
    | exception Invalid_argument _ ->
        ...
        let info =
          Printf.sprintf "%s exptects %d arguments, but you provided %d" name
            types_len arg_len
        in
        Errors.fail ~info
  in
  ...
  |> List.concat_map (fun (arg, ty) ->
         ...
         | Some inductive_name ->
             let recursor =
               List.combine inductive_names recursors
               |> List.assoc inductive_name
             in
             let r =
               Names.Id.to_string recursor ^ "_" ^ Names.Id.to_string arg
             in
...
let replace_recursor ~(recursors : Names.Id.t list) (c : Constrexpr.constr_expr)
    =
  let recursor_qualids =
    recursors |> List.map (fun recursor -> Libnames.qualid_of_ident recursor)
  in
...
let add_handler ~name ~arguments ~handler =
  ...
  | Some ty ->
      ...
      let handler =
        ...
        | Some arguments ->
            let inductive_names =
              recursion_ctx.inductive_paths |> List.map Naming.extract_path_base
            in
            let arguments =
              extend_argumets_with_inductive_case ~inductive_names
                ~recursors:recursion_ctx.names ~constructor:name ~arguments
...
let add_wildcard_handler ~handler =
  ...
  let cases =
    ...
    |> List.map (fun constructor ->
           let types =
             Termutils.flatten_inductive_constructor_type ~inductive_names
           ...
           in
           ( constructor,
             types |> List.map (fun _ -> Naming.fresh_name ~prefix:"arg") ))
  in
  cases
...
let elegant name (args : (Names.Id.t * Constrexpr.constr_expr) list) =
  ...
  open_recursion ~args ~suffix ~arguments:(List.map fst middle)
