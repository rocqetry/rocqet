...
let add ~(inductive_path : Libnames.qualid)
    ~(inherited_handlers : Names.Id.t list) ~(handlers : Names.Id.t list) =
  ...
  let ty =
    Termutils.compute_partial_recursor_signature ~inductive_path ~context
  in
...
let extend ~(inductive_path : Libnames.qualid)
    ~(inherited_handlers : Names.Id.t list) ~(handlers : Names.Id.t list) =
