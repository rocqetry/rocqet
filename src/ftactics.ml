...
let fsimpl () =
  Proofview.Goal.enter begin fun gl ->    
    ...
    let handlers =       
      ...
      |> List.filter_map (fun name -> 
           name |> Env.Context.lookup_linkage_elem context |> Option.map fst)
...
(* let h : (unit, unit) Context.Named.pt = failwith ""*)
(*
      ...
      |> List.concat_map 
           (fun (binding : (Evd.econstr, Evd.econstr) Context.Named.Declaration.pt) -> 
             ...
             | Context.Named.Declaration.LocalAssum ({ binder_name; _ }, ty) -> 
                Feedback.msg_notice (Pp.str (Names.Id.to_string binder_name));                
                [ty]
